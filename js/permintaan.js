...
/* ============================================================
   ...
   ============================================================ */
var _mhsActiveLoansCache = [];   // cache list alat yang sedang dipinjam, dipakai saat "Semua Alat" dipilih
var RET_ALL_VALUE = '__SEMUA_ALAT__';
...
async function loadMhsActiveLoans() {
  ...
  try {
    ...
    var totalPcs = list.reduce(function(sum,a){ return sum + (Number(a.jumlah)||0); }, 0);
    var optSemua = list.length > 1
      ? '<option value="'+RET_ALL_VALUE+'">— Semua Alat ('+list.length+' jenis, '+totalPcs+' pcs) —</option>'
      : '';
    if(sel) sel.innerHTML='<option value="">— Pilih Alat —</option>'+optSemua+list.map(function(a){ return '<option value="'+esc(a.nama)+'" data-jml="'+a.jumlah+'">'+esc(a.nama)+' (dipinjam: '+a.jumlah+')</option>'; }).join('');
    if(displayEl){
...
function fillReturnAlatInfo() {
  ...
  if(sel.value === RET_ALL_VALUE){
    var totalPcs = _mhsActiveLoansCache.reduce(function(sum,a){ return sum + (Number(a.jumlah)||0); }, 0);
    if(rj){ rj.value=totalPcs; rj.disabled=true; }
    if(ri){
      var daftar = _mhsActiveLoansCache.map(function(a){ return esc(a.nama)+' ('+a.jumlah+' pcs)'; }).join(', ');
      ri.innerHTML = 'Akan mengajukan pengembalian <strong>seluruh alat</strong> yang dipinjam sekaligus, masing-masing dalam jumlah penuh: '+daftar+'.';
    }
    return;
  }
...
async function submitReturnReq() {
  ...
  if(nama === RET_ALL_VALUE){
    return submitReturnReqAll(kondisi, catatan);
  }
...
/**
 * Ajukan pengembalian SEMUA alat yang sedang dipinjam sekaligus.
 * Mengirim satu submitReturnRequest per jenis alat (masing-masing
 * dalam jumlah penuh sesuai yang dipinjam), lalu tampilkan ringkasan
 * berapa yang berhasil & berapa yang gagal (mis. karena sudah ada
 * pengajuan menunggu untuk alat itu).
 */
async function submitReturnReqAll(kondisi, catatan) {
  var list = _mhsActiveLoansCache;
  if(!list || !list.length){ Swal.fire('Peringatan','Tidak ada alat yang dipinjam','warning'); return; }

  var daftarHtml = list.map(function(a){ return '<div style="padding:3px 0;">• '+esc(a.nama)+' — <strong>'+a.jumlah+' pcs</strong></div>'; }).join('');
  var r=await Swal.fire({
    title: 'Ajukan Pengembalian Semua Alat?',
    html: '<div style="text-align:left;font-size:13px;max-height:240px;overflow-y:auto;">'
        + 'Akan mengajukan pengembalian '+list.length+' jenis alat berikut (jumlah penuh):<br><br>'
        + daftarHtml
        + '<br>Kondisi: <strong>'+esc(kondisi)+'</strong></div>',
    icon: 'question',
  ...
  Swal.fire({title:'Mengirim '+list.length+' pengajuan...',allowOutsideClick:false,didOpen:function(){Swal.showLoading();}});

  var success=0, errors=[];
  for(var i=0;i<list.length;i++){
    var a=list[i];
    try {
      var res=await callGAS('submitReturnRequest',{nim:_uname,namaAlat:a.nama,jumlahKembali:a.jumlah,kondisi:kondisi,catatan:catatan});
      if(res && res.success) success++;
      else errors.push(a.nama+': '+(res&&res.message?res.message:'Gagal'));
    } catch(e){ errors.push(a.nama+': '+e.message); }
  }

  Swal.close();
  document.getElementById('retAlatNama').value=''; document.getElementById('retJumlah').value=''; document.getElementById('retJumlah').disabled=false;
  document.getElementById('retCatatan').value=''; document.getElementById('retKondisi').value='Baik';

  if(!errors.length){
    Swal.fire({icon:'success',title:'Semua Pengajuan Terkirim!',text:success+' alat berhasil diajukan pengembaliannya, menunggu konfirmasi admin.',timer:3500,showConfirmButton:false});
  } else {
    Swal.fire({
      icon: success>0 ? 'warning' : 'error',
      title: success+' berhasil, '+errors.length+' gagal',
      html: '<div style="font-size:12.5px;text-align:left;">'+errors.map(function(e){ return '<div style="color:#991b1b;">• '+esc(e)+'</div>'; }).join('')+'</div>'
    });
  }
  loadMhsActiveLoans(); loadReturnHistory(); loadMhsSummary();
}
