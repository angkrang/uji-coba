...
function _renderDosenMahasiswa(list) {
  ...
  body.innerHTML = list.map(function(m) {
    ...
    var sisaHtml = habis
      ...
      : esc(m.sisaWaktu || '-');
    return '<tr style="cursor:pointer;" onclick="_dosenOpenStudentProfile(\'' + esc(m.nim) + '\')" '      + 'onmouseover="this.style.background=\'#f0f9ff\'" onmouseout="this.style.background=\'\'">'      + '<td ' + td + ' style="font-size:12px;">' + esc(m.nimLengkap || m.nim) + '</td>'      + '<td ' + td + ' style="color:var(--primary,#2563eb);font-weight:600;">' + esc(m.nama) + '</td>'
      ...
      + '<td ' + td + '>'
        ...
        + 'onclick="event.stopPropagation();prefillDosenForm(\'' + esc(m.nim) + '\')">'
...
function _renderDosenRiwayat(list) {
  ...
  body.innerHTML = filtered.map(function(l) {
    ...
    return '<tr style="cursor:pointer;" onclick="_dosenOpenStudentProfile(\'' + esc(l.nim) + '\')" '      + 'onmouseover="this.style.background=\'#f0f9ff\'" onmouseout="this.style.background=\'\'">'      + '<td ' + td + ' style="white-space:nowrap;">' + esc(l.tanggal) + '</td>'      + '<td ' + td + '><strong style="color:var(--primary,#2563eb);">' + esc(l.namaMahasiswa) + '</strong><br>'
      ...
      + '<td ' + td + ' style="white-space:nowrap;">'
        + (l.statusKendala === 'Open'
          ? '<button class="btn btn-xs btn-outline" style="margin-right:4px;" onclick="event.stopPropagation();resolveKendala(\'' + esc(l.id) + '\')" title="Tandai kendala selesai"><i class="bi bi-check2"></i></button>'
          : '')
        + '<button class="btn btn-xs btn-outline" style="margin-right:4px;" onclick="event.stopPropagation();editDosenSesi(\'' + esc(l.id) + '\')" title="Edit sesi"><i class="bi bi-pencil"></i></button>'
        + '<button class="btn btn-xs" style="background:#fee2e2;color:#dc2626;border:none;border-radius:6px;padding:3px 8px;cursor:pointer;" onclick="event.stopPropagation();deleteDosenSesi(\'' + esc(l.id) + '\')" title="Hapus sesi"><i class="bi bi-trash3"></i></button>'
      + '</td>'
      + '</tr>';
  }).join('');
}
...
/* ============================================================
   PROFIL MAHASISWA — klik nama di tabel Bimbingan / Riwayat Sesi
   Menggunakan fungsi loadStudentDetail() dari dashboard.js yang
   sudah menangani tampilan lengkap (identitas, bon, peminjaman).

   nim yang datang bisa berupa:
   - Format pendek  : "551389"          (dari BIMBINGAN_LOG kolom B)
   - Format panjang : "26/551389/TP/08569" atau "26551389" (dari DataEdit)
   loadStudentDetail() + _normalizeNimShort() sudah menangani semua format,
   tapi kita pastikan NIM tidak kosong sebelum diteruskan.
   ============================================================ */
function _dosenOpenStudentProfile(nim) {
  if (!nim || nim.toString().trim() === '') {
    Swal.fire('Info', 'NIM mahasiswa tidak tersedia.', 'info');
    return;
  }
  if (typeof loadStudentDetail === 'function') {
    // Scroll ke atas agar transisi section terlihat
    window.scrollTo({ top: 0, behavior: 'smooth' });
    loadStudentDetail(nim.toString().trim());
    // Patch tombol kembali → mengarah ke Bimbingan Saya (sec-dosen)
    // Diberi sedikit delay agar DOM sec-student-detail sudah visible
    setTimeout(_dosenPatchBackButton, 80);
  } else {
    Swal.fire('Info', 'Fitur profil mahasiswa tidak tersedia di halaman ini.', 'info');
  }
}

/* ============================================================
   OVERRIDE TOMBOL "KEMBALI" DI sec-student-detail
   Saat profil dibuka dari menu Bimbingan Saya, tombol kembali
   harus mengarah ke sec-dosen (bukan ke Dashboard).
   Dipanggil dari _dosenOpenStudentProfile(), di-undo otomatis
   saat user klik tombol kembali.
   ============================================================ */
function _dosenPatchBackButton() {
  var btn = document.querySelector('#sec-student-detail button[onclick=\"goTo(\'dash\')\"]');
  if (!btn) return;

  // Simpan onclick asli supaya bisa di-restore
  btn._origOnclick = btn.getAttribute('onclick');
  btn.innerHTML    = '<i class="bi bi-arrow-left"></i> Kembali ke Bimbingan Saya';
  btn.setAttribute('onclick', '_dosenRestoreBackButton(); goTo(\'dosen\');');
}

function _dosenRestoreBackButton() {
  var btn = document.querySelector('#sec-student-detail button[onclick*="dosenRestoreBackButton"]');
  if (btn && btn._origOnclick) {
    btn.innerHTML = '<i class="bi bi-arrow-left"></i> Kembali ke Dashboard';
