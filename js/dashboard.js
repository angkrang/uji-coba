...
async function loadAdminDash() {
  try {
    ...
    // Count-up animation untuk stat cards
    function setCountUp(id, v) {
      var el = document.getElementById(id); if (!el) return;
      var num = parseInt(v, 10);
      if (!isNaN(num) && num > 0) _animateCountUp(el, num, 850);
      else if (el) el.textContent = v !== undefined ? v : 0;
    }
    setCountUp('st-bahan',  s.totalBahan    || 0);
    setCountUp('st-alat',   s.totalAlat     || 0);
    setCountUp('st-kritis', s.totalKritis   || 0);
    setCountUp('st-mhs',    s.statMhs       || 0);
    setCountUp('st-pay',    s.pendingPayment || 0);
...
async function loadStudentDetail(nim) {
  ...
  try {
    /* Normalisasi NIM ke format pendek (6 digit) agar cocok dengan
       data di CHEMICAL_HISTORY dan EQUIPMENT_HISTORY yang memakai
       format pendek, meski nim asli dari tabel bisa format panjang
       (mis: "26/551389/TP/08569" atau "26551389"). */
    var nimShortForQuery = _normalizeNimShort(nim);
    var results=await Promise.all([
      callGAS('getAllBorrowings'),
      callGAS('getMahasiswaChemicalLoans',{nim:nimShortForQuery}),
      callGAS('getStudentEquipmentHistory',{nim:nimShortForQuery}).catch(function(){return[];})
    ]);
...
/* ============================================================
   ...
   ============================================================ */
var _bonEditRegistry = []; // Reset setiap kali tabel di-render ulang

function _renderDetailBon(data) {
  _bonEditRegistry = []; // Reset registry tiap render
  ...
  var rows=filtered.map(function(r){
    ...
    var canEdit=(_role==='admin'||_role==='plp')&&r.idReq;
    var bonIdx = _bonEditRegistry.length;
    if (canEdit) _bonEditRegistry.push({idReq:r.idReq,namaBahan:r.namaBahan,jumlah:r.jumlah,tipe:r.tipe,satuan:satuan});
    var btnEdit=canEdit
      ?'<td style="width:60px;"><button class="btn btn-xs" style="background:#dbeafe;color:#1e40af;border:none;cursor:pointer;border-radius:6px;font-size:11px;font-weight:600;padding:3px 8px;white-space:nowrap;" '
        +'onclick="openEditBon('+bonIdx+')"><i class="bi bi-pencil"></i> Edit</button></td>'
      :'<td></td>';
    return '<tr>'
  ...
  var hasEdit=(_role==='admin'||_role==='plp');
  el.innerHTML='<div class="tbl-wrap"><table class="tbl"><thead><tr>'
    ...
    +(hasEdit?'<th style="min-width:60px;">Aksi</th>':'<th></th>')
    +'</tr></thead><tbody>'+rows+'</tbody>'
    +'<tfoot><tr style="background:#dbeafe;">'
    +'<td colspan="'+(hasEdit?9:8)+'" style="text-align:right;padding:8px;font-weight:bold;font-size:12px;">TOTAL BIAYA</td>'
...
/* ============================================================
   EDIT BON — Admin & PLP
   Membuka modal untuk mengubah nama bahan atau jumlah bon
   ============================================================ */
var _editBonData = null;

function openEditBon(idx) {
  var bonData = _bonEditRegistry[idx];
  if (!bonData) return;
  _editBonData = bonData;
  var el = document.getElementById('mdlEditBon');
  if (!el) return;
  document.getElementById('editBonIdReq').textContent   = bonData.idReq || '—';
  document.getElementById('editBonNama').value          = bonData.namaBahan || '';
  document.getElementById('editBonJumlah').value        = bonData.jumlah || '';
  document.getElementById('editBonSatuan').textContent  = bonData.satuan || '';
  document.getElementById('editBonTipe').textContent    = bonData.tipe || '—';
  openModal('mdlEditBon');
}

async function saveEditBon() {
  if (!_editBonData) return;
  var namaBaru   = (document.getElementById('editBonNama').value   || '').trim();
  var jumlahBaru = parseFloat(document.getElementById('editBonJumlah').value);
  if (!namaBaru)              { Swal.fire('Peringatan','Nama bahan tidak boleh kosong','warning'); return; }
  if (isNaN(jumlahBaru)||jumlahBaru<=0) { Swal.fire('Peringatan','Jumlah harus berupa angka positif','warning'); return; }
  var r = await Swal.fire({
    title:'Simpan Perubahan Bon?',
    html:'<b>No. Bon:</b> '+esc(_editBonData.idReq)+'<br>'
        +'<b>Nama:</b> '+esc(namaBaru)+'<br>'
        +'<b>Jumlah:</b> '+jumlahBaru+' '+esc(_editBonData.satuan||''),
    icon:'question', showCancelButton:true,
    confirmButtonText:'Simpan', cancelButtonText:'Batal', confirmButtonColor:'#1d4ed8'
  });
  if (!r.isConfirmed) return;
  Swal.fire({title:'Menyimpan...',allowOutsideClick:false,didOpen:function(){Swal.showLoading();}});
  try {
    var res = await callGAS('editBonMahasiswa',{
      idReq     : _editBonData.idReq,
      namaBahan : namaBaru,
      jumlah    : jumlahBaru,
      adminNim  : _uname
    });
    Swal.close();
    if (res && res.success) {
      closeModal('mdlEditBon');
      Swal.fire({toast:true,position:'top-end',icon:'success',title:'Data bon berhasil diubah',showConfirmButton:false,timer:2000});
      /* Refresh detail halaman mahasiswa */
      if (_detailNim) loadStudentDetail(_detailNim);
    } else {
...
function _animateCountUp(el, target, duration) {
  if (!el) return;
  var isNum = typeof target === 'number' && !isNaN(target);
...
/* ============================================================
   CUSTOM EMPTY STATE SVG ICONS
   ============================================================ */
var _emptyStateSVGs = {
  lab: '<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" width="36" height="36">'
    + '<path d="M18 6h12v16l8 18H10L18 22V6z" stroke="#94a3b8" stroke-width="2.5" stroke-linejoin="round" fill="#dbeafe" fill-opacity=".6"/>'
    + '<path d="M15 30h18M18 22h12" stroke="#2563eb" stroke-width="2" stroke-linecap="round"/>'
    + '<circle cx="20" cy="34" r="2" fill="#7c3aed" opacity=".7"/>'
    + '<circle cx="27" cy="37" r="1.5" fill="#f97316" opacity=".7"/>'
    + '</svg>',

  tools: '<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" width="36" height="36">'
    + '<rect x="8" y="20" width="12" height="24" rx="3" fill="#dcfce7" stroke="#166534" stroke-width="2"/>'
    + '<rect x="28" y="14" width="12" height="30" rx="3" fill="#dcfce7" stroke="#166534" stroke-width="2"/>'
    + '<path d="M14 20V10a6 6 0 0 1 12 0v10" stroke="#059669" stroke-width="2.5" stroke-linecap="round"/>'
    + '</svg>',

  activity: '<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" width="36" height="36">'
    + '<rect x="6" y="8" width="36" height="32" rx="6" fill="#ede9fe" stroke="#7c3aed" stroke-width="2"/>'
    + '<path d="M12 18h24M12 26h16M12 34h10" stroke="#7c3aed" stroke-width="2.5" stroke-linecap="round"/>'
    + '<circle cx="36" cy="12" r="6" fill="#f97316"/>'
    + '<path d="M36 9v4l2 2" stroke="#fff" stroke-width="1.5" stroke-linecap="round"/>'
    + '</svg>',

  ok: '<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" width="36" height="36">'
    + '<circle cx="24" cy="24" r="18" fill="#dcfce7" stroke="#166534" stroke-width="2"/>'
    + '<path d="M14 24l7 7 13-13" stroke="#166534" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>'
    + '</svg>',
};

/* Inject custom SVG empty states ke semua empty-state-icon yang pakai emoji bahan/alat */
function patchEmptyStateIcons() {
  document.querySelectorAll('.empty-state').forEach(function(es) {
    var iconEl = es.querySelector('.empty-state-icon');
    if (!iconEl) return;
    var txt = iconEl.textContent.trim();
    var svg = null;
    if (txt === '🧪') svg = _emptyStateSVGs.lab;
    else if (txt === '🔧') svg = _emptyStateSVGs.tools;
    else if (txt === '📋') svg = _emptyStateSVGs.activity;
    else if (txt === '✅') svg = _emptyStateSVGs.ok;
    if (svg) {
...
// Patch setelah DOM stabil
document.addEventListener('DOMContentLoaded', function() {
  setTimeout(patchEmptyStateIcons, 2000);
});

/* Observer untuk patch dynamic empty states */
var _emptyObs = new MutationObserver(function() {
  patchEmptyStateIcons();
});
document.addEventListener('DOMContentLoaded', function() {
  var mc = document.getElementById('mainContent');
  if (mc) _emptyObs.observe(mc, { childList: true, subtree: true });
});
