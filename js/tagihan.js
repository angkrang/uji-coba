...
function _renderFormulirTable(nama, nim, prodi, items, grandTotal) {
  ...
  var rows = items.map(function(d, i) {
    return '<tr>'
      ...
      + '<td style="text-align:right;white-space:nowrap;padding:6px 8px;">' + formatRupiah(d.harga) + '</td>'
      + '<td style="text-align:right;white-space:nowrap;padding:6px 8px;">' + formatRupiah(d.total) + '</td>'
      + '</tr>';
  }).join('');
  ...
  return ''
    ...
    + '<th style="padding:7px 8px;border:1px solid #b0b8c9;text-align:center;width:110px;font-size:12px;">Jumlah</th>'
    + '<th style="padding:7px 8px;border:1px solid #b0b8c9;text-align:right;width:130px;font-size:12px;white-space:nowrap;">Harga/Satuan</th>'
    + '<th style="padding:7px 8px;border:1px solid #b0b8c9;text-align:right;width:130px;font-size:12px;white-space:nowrap;">Jumlah Harga</th>'
    + '</tr>'
    + '</thead>'
    + '<tbody>'
    + rows
    ...
    + '<td style="padding:8px;border:1px solid #b0b8c9;text-align:right;font-weight:700;font-size:13px;white-space:nowrap;">' + formatRupiah(grandTotal) + '</td>'
    + '</tr>'
    + '</tfoot>'
    + '</table>'
...
async function loadPaySummary() {
  ...
  try {
    /* Selalu ambil ulang harga terbaru dari Sheet setiap kali halaman
       Tagihan dibuka/di-refresh — JANGAN cache di _hargaMap antar
       pemanggilan, karena admin bisa update harga bahan kapan saja
       dan mahasiswa harus selalu lihat angka terbaru saat mengajukan
       tagihan (dulu ada guard "if kosong baru fetch" yang bikin harga
       lama/nol nyangkut terus sepanjang sesi browser walau sudah klik
       tombol "Refresh"). */
