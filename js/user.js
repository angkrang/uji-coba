...
async function loadUsers() {
  ...
  try {
    ...
    if(!data||!data.length){ if(tb) tb.innerHTML='<tr><td colspan="4"><div class="empty-state"><div class="empty-state-icon">👤</div><div class="empty-state-title">Belum ada user</div></div></td></tr>'; return; }
    if(tb) tb.innerHTML=data.map(function(u){
      var rb=u.role.toLowerCase()==='admin'
        ?'<span class="badge b-purple"><i class="bi bi-shield-check" style="margin-right:3px;"></i>'+esc(u.role)+'</span>'
        :u.role.toLowerCase()==='dosen'
        ?'<span class="badge b-orange"><i class="bi bi-person-workspace" style="margin-right:3px;"></i>'+esc(u.role)+'</span>'
        :'<span class="badge b-blue"><i class="bi bi-person" style="margin-right:3px;"></i>'+esc(u.role)+'</span>';
      var delBtn=u.username!==_uname
        ?'<button class="btn btn-xs" style="background:#fee2e2;color:#991b1b;border:none;cursor:pointer;border-radius:6px;font-size:11px;font-weight:600;" onclick="delUser(\''+esc(u.username)+'\')"><i class="bi bi-trash"></i></button>'
        :'<span style="color:var(--muted);font-size:12px;">(aktif)</span>';
      return '<tr class="user-row" data-s="'+esc((u.username+u.nama).toLowerCase())+'">'
        +'<td><code>'+esc(u.username)+'</code></td>'
        +'<td style="font-weight:600;">'+esc(u.nama)+'</td>'
        +'<td>'+rb+'</td>'
        +'<td style="display:flex;gap:4px;flex-wrap:wrap;">'
        +'<button class="btn btn-xs btn-outline" onclick="openEditUser(\''+esc(u.username)+'\',\''+esc(u.nama)+'\',\''+esc(u.role)+'\')"><i class="bi bi-key"></i> Reset Pass</button>'
        +delBtn+'</td></tr>';
    }).join('');
  } catch(e) {}
}
...
function openModalAddUser() {
  ...
  ['uUsername','uNama','uPass','uDosen','uJudul','uTglMulai','uTglSelesai'].forEach(function(id){ var el=document.getElementById(id); if(el) el.value=''; });
...
function openEditUser(username, nama, role) {
...
async function doSaveUser() {
  ...
  if (!_editUsername) {
    ...
    try {
      ...
      var addRes = await callGAS('addUser', { username: username, password: pass, nama: nama, role: role });
      Swal.close();
  ...
  /* ── MODE EDIT USER ── */
    ...
    if (role === 'Mahasiswa') {
      try {
        ...
        if (saveRes.success) {
          ...
          if (typeof _detailNim !== 'undefined' && _detailNim === username && typeof loadStudentDetail === 'function') {
    ...
    try {
      ...
      var editRes = await callGAS('updateUserInfo', { username: username, nama: nama, role: role });
      Swal.close();
